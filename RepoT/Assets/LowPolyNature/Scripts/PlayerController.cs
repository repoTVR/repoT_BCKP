...
public class PlayerController : MonoBehaviour
{
    ...
    private Vector3 targetFinale;
    private float startTime;
    private float time;
    private float distanza;
    private float distCovered;
    private float percDist;
    bool pressed;
    float tempoVolo = 5f;
    float elevazione = 5f;
    public GameObject cuboSucc;
    public GameObject cuboIniz;
    float dist;
    #endregion
    ...
    void Start()
    {
        dist = Mathf.Abs(cuboSucc.transform.position.z - cuboIniz.transform.position.z);
        
        Debug.Log("Dist = " + dist);
        posIniziale = Vector3.zero;
        target = Vector3.zero;
        targetFinale = Vector3.zero;
        distanza = 0f;
    ...
    void Update()
    {
        if (!IsDead && mIsControlEnabled)
        {
            ...
            if (_characterController.isGrounded)
            {
                ...
                {
                    Debug.Log("Pos iniziale" + transform.position);
                    pressed = true;
                    _animator.SetBool("is_in_air", true);
                    _moveDirection.y = JumpSpeed;
                    _moveDirection.z = dist;
                    //StartCoroutine("jump");
    ...
    //IEnumerator jump()
    //{
    //    Vector3 posPrecisa = Vector3.zero;
    //    startTime = Time.time;
    //    posIniziale = transform.position;
    //    float timeStartedLearping = 0f;
    //    float lerpTime = 5.0f;
    //    float percTime = 0f;
    //    target = new Vector3(posIniziale.x, posIniziale.y + elevazione, posIniziale.z + 5f);
    //    targetFinale = new Vector3(target.x, posIniziale.y, target.z + 5f);
    //    //distanza = Vector3.Distance(posIniziale, target);
    //    //distCovered = (Time.time - startTime) * 1.0f;
    //    //percDist = distCovered / distanza;


    //    while (lerpTime >= 0f)
    //    {
    //        lerpTime -= Time.deltaTime;
    //        percTime = 1 - (lerpTime / tempoVolo);
    //        if( lerpTime >= tempoVolo/2)
    //        {
                
    //            transform.position = Vector3.Lerp(posIniziale, target, (percTime*2));
    //            Debug.Log("1 Lerp time = " + lerpTime);
    //            Debug.Log("1 Vec" + transform.position);
    //            posPrecisa = transform.position;


    //        }
    //        else //if(lerpTime < lerpTime/2 && lerpTime > 0f)
    //        {
    //            //lerpTime = (float) System.Math.Round(lerpTime, 2);
    //            Debug.Log("2 Lerp time = " + lerpTime);
    //            Debug.Log("2 Vec" + transform.position);
    //            transform.position = Vector3.Lerp(posPrecisa, targetFinale, percTime);
    //        }
